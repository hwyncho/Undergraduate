...
void MYError(const char * str, FILE * fp, int fd)
{
	printf("%s", str);
...
void test_case_1()
{
	...
	char * TC__FILE_NAME = (char *)malloc(32 * sizeof(char));
	sprintf(TC__FILE_NAME, "%d.txt", getpid());
...
int main(int argc, char * argv[])
{
	...
	while (runProcess < TOTALFORK)
	{
		...
		else if (pids[runProcess] == 0)
		{
			...
			waitpid(pids[runProcess], &status, 0);
